...
- `scripts/forensic_residue.py` — dependency-free cleanup for pattern #40 only: chatbot citation tokens, chatbot `utm_source` parameters, invisible characters, and homoglyphs inside Latin words. It is a tool for this repo and for drafts, not part of the skill. `SKILL.md` must never mention it (see the maintenance contract), so the single-file copies embedded in downstream apps lose nothing.
- `scripts/test_forensic_residue.py` — its tests, including a guard that the residue scan finds nothing in this repo's own Markdown.
...
- **Self-contained prompt:** `SKILL.md` may not reference anything in `scripts/`, and `validate-package.py` fails if it does. The skill runs from a single file in harnesses and apps that never see this repository, so a line telling the agent to run a script would be dead weight there, paid for on every invocation. Tools in `scripts/` are for the maintainer and for drafts on disk, and a change confined to `scripts/` does not bump the skill version, which tracks the prompt.
...
- **Validation:** run `python3 scripts/validate-package.py`, `python3 -m unittest scripts/test_forensic_residue.py`, `npx skills add . --list`, and `claude plugin validate .` before publishing.
- **Upstream:** this fork has diverged on purpose. Upstream `blader/humanizer` rewrote its prompt in Plain Language for 2.11.0 and renumbered to 35 patterns, then rebuilt it again for 3.0.0 (2026-09-06) around 25 patterns ordered by strength, so a plain `git merge origin/main` is not the way to take upstream work and would now drop #26-54; port the individual patterns and packaging fixes and record what was skipped in the README version history. Upstream also trims trigger phrases it judges "already implied" by a pattern's problem statement; keep those phrases here, because this fork's releases keep showing that a right rule with a short trigger list matches nothing. Both repos also used the numbers 2.11.0 and 2.11.1 for unrelated releases, so upstream version numbers are not comparable to this fork's.
